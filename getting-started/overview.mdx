...
<Info>
  Authentication is required in order to access the platform.
</Info>
...
Powered by GPT-5.2 and retrieval augmented generation (RAG) technology, the "Support Agent" is an AI agent that draws from a comprehensive knowledge base curated by the BYU-Idaho Support Center's staff to provide accurate, grounded responses based on official knowledge sources.
...
```mermaid
%%{init: {'theme':'base', 'themeVariables': { 'primaryColor':'#1e3a8a','primaryTextColor':'#fff','primaryBorderColor':'#1e40af','lineColor':'#3b82f6','secondaryColor':'#0f172a','tertiaryColor':'#1e293b','background':'transparent'}}}%%
  flowchart LR
    Human["fa:fa-user Human<br/><small>Student or Employee</small>"]
    Guardrail["fa:fa-shield-alt Guardrail Agent<br/><small>Privacy & Safety Monitor</small>"]
    Support["fa:fa-robot Support Agent<br/><small>Frontline Support</small>"]
    Research["fa:fa-search Research Agent<br/><small>Specialized Research</small>"]

    Human -->|Asks Question| Guardrail
    Guardrail -->|Monitors Input| Support
    Support -->|Needs Research| Research
    Research -->|Provides Information| Support
    Support -->|Generates Response| Guardrail
    Guardrail -->|Monitors Output| Human

    style Human fill:#1e3a8a,stroke:#1e40af,stroke-width:2px,color:#fff
    style Guardrail fill:#0f172a,stroke:#1e40af,stroke-width:2px,color:#fff
    style Support fill:#1e3a8a,stroke:#1e40af,stroke-width:2px,color:#fff
    style Research fill:#1e293b,stroke:#1e40af,stroke-width:2px,color:#fff
```
...
1. **Visit the website**: Go to https://supportagent.byui.edu
